...
// Function to clean up auth state only when explicitly needed
const cleanupAuthState = () => {
  console.log('Cleaning up auth state...');
  // Only clean up specific auth keys, be more selective
  const keysToRemove = [];
  
  Object.keys(localStorage).forEach((key) => {
    if (key.startsWith('supabase.auth.token') || key.includes('sb-ecziljpkvshvapjsxaty-auth-token')) {
...
// Export helper function for manual cleanup only
