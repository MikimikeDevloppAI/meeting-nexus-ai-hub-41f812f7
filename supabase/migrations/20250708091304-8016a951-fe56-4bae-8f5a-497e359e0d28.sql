-- Fix the trigger to preserve manually set exchange rates
...
BEGIN
  ...
  -- 2. GESTION DU TAUX DE CHANGE - VERSION CORRIGÉE QUI PRESERVE LES VALEURS MANUELLES
  
  -- Cas 1: Nouvelle facture avec devise CHF ET pas de taux de change
  ...
  -- Cas 2: Nouvelle facture avec autre devise ET pas de taux de change
  ...
  -- Cas 3: Modification de devise vers CHF ET pas de taux valide
  ELSIF TG_OP = 'UPDATE' AND OLD.currency != NEW.currency AND NEW.currency = 'CHF' AND (NEW.exchange_rate IS NULL OR NEW.exchange_rate <= 0) THEN
    NEW.exchange_rate := 1;
    RAISE NOTICE 'TRIGGER DEBUG: Case 3 - UPDATE to CHF, setting exchange_rate to 1';
  
  -- Cas 4: Modification de devise vers autre chose que CHF ET pas de taux valide
  ELSIF TG_OP = 'UPDATE' AND OLD.currency != NEW.currency AND NEW.currency != 'CHF' AND (NEW.exchange_rate IS NULL OR NEW.exchange_rate <= 0) THEN
    should_call_api := TRUE;
    RAISE NOTICE 'TRIGGER DEBUG: Case 4 - UPDATE to non-CHF, will call API';
  
  -- NOUVEAU CAS 5: Si on a déjà un taux valide, ON NE TOUCHE PAS
  ELSIF NEW.exchange_rate IS NOT NULL AND NEW.exchange_rate > 0 THEN
    RAISE NOTICE 'TRIGGER DEBUG: Preserving existing valid exchange rate %', NEW.exchange_rate;
  ...
  -- Appel API SEULEMENT si nécessaire ET qu'on n'a pas déjà un taux valide
